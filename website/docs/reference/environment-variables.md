...
| `BING_SEARCH_API_KEY` | Bing Web Search API key for search-only fallback/provider use ([bing.microsoft.com](https://www.microsoft.com/bing/apis/bing-web-search-api)) |
| `BING_SEARCH_ENDPOINT` | Custom Bing Web Search endpoint (optional; defaults to `https://api.bing.microsoft.com/v7.0/search`) |
| `SERPAPI_API_KEY` | SerpAPI key for search-only fallback/provider use ([serpapi.com](https://serpapi.com/)) |
| `SERPAPI_ENDPOINT` | Custom SerpAPI endpoint (optional; defaults to `https://serpapi.com/search.json`) |
